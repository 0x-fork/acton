      ...
      <section className={styles.card} aria-labelledby="locker-overview-title">
        <div className={styles.header}>
          ...
          <p className={styles.description}>
            {schedule.totalPeriods} payments every {formatPeriod(data.unlockPeriod)}, from{" "}
            {firstPayment ? formatDate(firstPayment.unlockTime) : "—"} to{" "}
            {finalPayment ? formatDate(finalPayment.unlockTime) : "—"}
          </p>
        </div>
        ...
        <div className={styles.metrics}>
          ...
          <LockerMetric
            label="Next payment"
            value={schedule.nextPayment ? formatDate(schedule.nextPayment.unlockTime) : "Completed"}
          />
        </div>
...
function LockerPaymentRow({payment}: {readonly payment: LockerPayment}) {
  return (
    <DataTableRow selected={payment.status === "next"}>
      ...
      <DataTableCell>{formatDate(payment.unlockTime)}</DataTableCell>
...
function formatGramAmount(value: bigint, maximumFractionDigits = 2): string {
...
function formatTimeUntil(timestamp: number, nowSeconds: number): string {
