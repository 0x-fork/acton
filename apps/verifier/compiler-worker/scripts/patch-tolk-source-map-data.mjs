...
for (const version of TOLK_SOURCE_MAP_VERSIONS) {
  const packageDir = path.join(compilerWorkerDir, "node_modules", `tolk-${version}`, "dist");
...
function replaceInFile(filePath, version, search, replacement) {
  ...
  if (!current.includes(search)) {
    throw new Error(`Could not patch tolk-${version}: expected fragment not found in ${filePath}`);
  }
